...
const App = () => {
    ...
    useEffect(() => {
        fetch(`${process.env.REACT_APP_BASE_URL}/mtx/instructions`).then(res => res.json()).then(data => {
            setdata(data)
        }).catch(e => {
            // setdata(`err - $${e}`)
        });
    ...
    return (
        <div style={{background: 'white', width: '100%', height: '100%', overflow: 'hidden'}}>
            <div style={{ overflowY: 'scroll', width: '100%', height: '100%' }}>
                ...
                <div style={{fontSize:'1.2rem',fontWeight:700,paddingLeft:'5%',marginBottom:'5%',marginTop:'7%'}}>Instructions: </div>
                {data && data.map((instruction) => {
                        return <div style={{paddingLeft:'5%',paddingRight:'5%',marginBottom:'3%',fontSize:'1rem'}} dangerouslySetInnerHTML={{__html:instruction}}></div>
                    })}
